...
function articuloR($conn,$compania){
    ...
    $sql="SELECT COUNT(Factura.idArticulo), Factura.idArticulo, ArticuloExistente.descripcion FROM(((Factura JOIN ArticuloVendido ON Factura.folio=ArticuloVendido.folio) JOIN Cliente ON Cliente.idCliente=Factura.idCliente) JOIN ArticuloExistente ON ArticuloExistente.idArticulo=Factura.idArticulo)JOIN ReporteOrden ON ReporteOrden.folio=Factura.folio WHERE bloqueo=0 AND ArticuloExistente.estatus=1 AND Factura.estatus=1 AND ArticuloVendido.estatus=1 AND Cliente.estatus=1 AND ReporteOrden.estatus=1 AND Factura.idCompania=? GROUP BY idArticulo";
...
function clienteR($conn,$compania){
    //$sql="SELECT * FROM Cliente WHERE bloqueo=0 AND estatus=1";
    $sql="SELECT COUNT(Factura.idCliente), Factura.idCliente, Cliente.nombreCliente FROM(((Factura JOIN ArticuloVendido ON Factura.folio=ArticuloVendido.folio) JOIN Cliente ON Cliente.idCliente=Factura.idCliente) JOIN ArticuloExistente ON ArticuloExistente.idArticulo=Factura.idArticulo)JOIN ReporteOrden ON ReporteOrden.folio=Factura.folio WHERE bloqueo=0 AND ArticuloExistente.estatus=1 AND Factura.estatus=1 AND ArticuloVendido.estatus=1 AND Cliente.estatus=1 AND ReporteOrden.estatus=1 AND Factura.idCompania=? GROUP BY idCliente";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt,$sql))
    {
    header("location: ../php/index.php?error=stmtfailed");
...
function uventaR($conn,$compania){
    //$sql="SELECT * FROM Cliente WHERE bloqueo=0 AND estatus=1";
    $sql="SELECT COUNT(ArticuloVendido.udVta),ArticuloVendido.udVta FROM(((Factura JOIN ArticuloVendido ON Factura.folio=ArticuloVendido.folio) JOIN Cliente ON Cliente.idCliente=Factura.idCliente) JOIN ArticuloExistente ON ArticuloExistente.idArticulo=Factura.idArticulo)JOIN ReporteOrden ON ReporteOrden.folio=Factura.folio WHERE bloqueo=0 AND ArticuloExistente.estatus=1 AND Factura.estatus=1 AND ArticuloVendido.estatus=1 AND Cliente.estatus=1 AND ReporteOrden.estatus=1 AND Factura.idCompania=? GROUP BY udVta";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt,$sql))
    {
    header("location: ../php/index.php?error=stmtfailed");
...
function MaR($conn,$compania){
    //$sql="SELECT * FROM Cliente WHERE bloqueo=0 AND estatus=1";
    $sql="SELECT COUNT(Factura.fechaFac),Factura.fechaFac FROM(((Factura JOIN ArticuloVendido ON Factura.folio=ArticuloVendido.folio) JOIN Cliente ON Cliente.idCliente=Factura.idCliente) JOIN ArticuloExistente ON ArticuloExistente.idArticulo=Factura.idArticulo)JOIN ReporteOrden ON ReporteOrden.folio=Factura.folio WHERE bloqueo=0 AND ArticuloExistente.estatus=1 AND Factura.estatus=1 AND ArticuloVendido.estatus=1 AND Cliente.estatus=1 AND ReporteOrden.estatus=1 AND Factura.idCompania=? GROUP BY fechaFac";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt,$sql))
    {
    header("location: ../php/index.php?error=stmtfailed");
